 ...
 * User: sunshow
 */
public class FolsomMemcacheLock extends MemcacheLock {
     ...
     */
    public FolsomMemcacheLock(MemcacheClient<String> memcacheClient, String name, long timeout, TimeUnit timeUnit) {
    ...
    protected CasValue<String> casGetKey(String key) {
        try {
            GetResult<String> getResult = memcacheClient.casGet(key).get();
            if (getResult == null) {
                return null;
            }
            return new CasValue<>(getResult.getValue(), getResult.getCas());
        } catch (Exception e) {
